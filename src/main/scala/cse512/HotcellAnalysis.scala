...
def runHotcellAnalysis(spark: SparkSession, pointPath: String): DataFrame =
{
  ...
  spark.udf.register("getNeighbors",(x1: Int, y1: Int, z1:Int)=>
    HotcellUtils.getNeighbors(x1, y1, z1)
  )
  val withNeighbor = spark.sql("select x,y,z, attrVal, getNeighbors(x,y,z) as numOfNb,attrVal*getNeighbors(x,y,z) as sum_w from cells,group by x, y, z ")
  withNeighbor.createOrReplaceTempView("withNeighbor")
  withNeighbor.show()
  val ifNeighbor = spark.sql("select c1.x as x , c1.y as y, c1.z as z, c1.numOfNb, sum(c2.attrVal) as sumAttr from withNeighbor  group by c1.x,c1.y,c1.z,c1.numOfNb").persist()
  ifNeighbor.show()
  ...
  val sum = withNeighbor.map(r => {
    (r.getAs[Long](4) * r.getAs[Long](5))
  }).reduce((i, j) => i + j)
  println(sum)

  val withGscore =  spark.sql("select x,y,z,GScore("+ mean + ","+ s +",numOfNb,sum_w," + numCells+") as Gscore from withNeighbor")
