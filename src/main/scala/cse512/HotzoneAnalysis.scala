...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    val joinDfOrdered = spark.sql("select rectangle, count(*) from joinResult group by rectangle order by rectangle")
    joinDfOrdered.createOrReplaceTempView("joinResultOrdered")
    joinDf.show()
    joinDfOrdered.show()
    // YOU NEED TO CHANGE THIS PART
    return joinDfOrdered.coalesce(1)
